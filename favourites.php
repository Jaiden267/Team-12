...
<main class="cart-wrap">
    <div class="container">
        <section class="card">
            ...
            <div id="favEmpty" class="cart-empty" hidden>You have no favourites yet.</div>
...
function updateFavCountDisplay() {
    if (typeof loadFavourites !== 'function') return;
...
function renderFavourites() {
    ...
    items.forEach((item, index) => {
        ...
        card.innerHTML = `
            <div class="fav-image-wrap">
                <img src="${item.image || ''}" alt="${item.name || 'Product'}" class="fav-image">
            </div>
            <div class="fav-info">
                <h3>${item.name || 'Unnamed Product'}</h3>
                <p class="price">${fmt(item.price)}</p>
                ${item.attribute_value || item.size ? `<p><span class="muted">Size:</span> ${item.attribute_value || item.size}</p>` : ''}
                <div class="fav-actions">
                    <button class="btn add-to-cart-btn" data-index="${index}">Add to Basket</button>
                    <button class="link-btn remove-fav-btn" data-index="${index}">Remove</button>
                </div>
            </div>
        `;
    ...
    document.querySelectorAll('.remove-fav-btn').forEach(btn => {
        btn.addEventListener('click', () => {
            ...
            renderFavourites();
            if (typeof updateFavouriteButtons === 'function') updateFavouriteButtons();
        });
    });
        ...
        btn.addEventListener('click', () => {
            ...
            if (typeof addToCart === 'function') {
                addToCart({
                    ...
                    qty: 1,
                    image: item.image || '',
                    size: item.attribute_value || item.size || ''
                });
