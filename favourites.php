...
if ($is_logged_in) {
  ...
  $stmt = $conn->prepare("
  SELECT DISTINCT p.*, pi.image_url FROM favourites f JOIN product_variants pv ON f.variant_id = pv.variant_id JOIN products p ON pv.product_id = p.product_id LEFT JOIN product_images pi ON p.product_id = pi.product_id AND (pi.is_main = 1 OR pi.is_main IS NULL) WHERE f.user_id = ?");
...
<main>
    <div class="container">
        <h1 style="margin: 40px 0 20px 0;">Favourites</h1>
        <?php if (!$is_logged_in): ?>
          <p>Please log in to see your favourites.</p>
          <?php elseif ($result && $result->num_rows === 0): ?>
            <p>No products found in your favourites.</p>
            <?php else: ?>
